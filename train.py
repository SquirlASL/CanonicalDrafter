    ...
    for row in batch["traced_tactics"]:
        for tac in row:
            ...
            if tactic.startswith("have"):
                # Replace the proof with `by sorry`
                transformed = ("by".join(tactic.split("by")[:-1]) + "by sorry") if "by" in tactic else tactic
                inputs.append(gb)
            ...
            synthesized = f"have {hyp_text} : {goal_after.strip()} := by sorry"
            inputs.append(gb)
